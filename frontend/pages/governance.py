...
def fetch_audit_logs(user_filter=None, action_filter=None, start_date=None, end_date=None):
    try:
        r = requests.get(f"{API_BASE}/api/v1/governance/audit-logs", timeout=3)
...
def fetch_model_usage_gov():
    try:
        r = requests.get(f"{API_BASE}/api/v1/governance/model-usage", timeout=3)
        if r.status_code == 200:
            return r.json()
    except Exception:
        pass
...
def fetch_user_activity():
    try:
        r = requests.get(f"{API_BASE}/api/v1/governance/user-activity", timeout=3)
        if r.status_code == 200:
            return r.json()
    except Exception:
        pass
...
def fetch_compliance_data():
    try:
        r = requests.get(f"{API_BASE}/api/v1/governance/compliance", timeout=3)
        if r.status_code == 200:
            return r.json()
    except Exception:
        pass
...
def show():
    st.markdown('<div class="main-header">🛡️ Governance</div>', unsafe_allow_html=True)
    st.markdown(
    ...
    tab1, tab2, tab3, tab4 = st.tabs(
        ["📋 Audit Logs", "🧠 Model Usage", "👥 User Activity", "📄 Compliance Reports"]
    )
    ...
    with tab2:
        ...
        metric_card("Total Model Calls", f"{model_data['total_calls']:,}", icon="🧠")
        ...
        with chart_cols[0]:
            fig = go.Figure(
                data=[
                    go.Bar(
                        x=list(model_data["cost_by_model"].keys()),
                        y=list(model_data["cost_by_model"].values()),
        ...
        with chart_cols[1]:
            usage = model_data["usage_over_time"]
            fig = go.Figure(
                data=[
                    go.Scatter(
                        x=usage["dates"],
    ...
    with tab3:
        ...
        metric_card("Active Users (30d)", user_data["active_users"], delta=8, icon="👥")

        chart_cols = st.columns(2)
        with chart_cols[0]:
            top_df = pd.DataFrame(user_data["top_users"])
        ...
        with chart_cols[1]:
            fig = go.Figure(
                data=[
                    go.Pie(
                        labels=list(user_data["activity_by_role"].keys()),
                        values=list(user_data["activity_by_role"].values()),
    ...
    with tab4:
        ...
        overview = comp_data["overview"]

        comp_cols = st.columns(4)
        with comp_cols[0]:
            metric_card("Total Reports", overview["total_reports"], icon="📄")
        with comp_cols[1]:
            metric_card("Passed", overview["passed"], icon="✅")
        with comp_cols[2]:
            metric_card("Failed", overview["failed"], icon="❌")
        with comp_cols[3]:
            metric_card("Compliance Rate", f"{overview['compliance_rate']}%", delta=2.1, icon="📊")
        ...
        hist_df = pd.DataFrame(comp_data["history"])
